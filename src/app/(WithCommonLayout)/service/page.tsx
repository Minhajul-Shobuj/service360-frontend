            ...
            >
              ...
              >
                {filteredCategories.map((cat) => (
                  <li
                    key={cat}
                    className="min-w-[70vw] sm:min-w-[220px] lg:min-w-0"
                  >
                    <button
                      className={`w-full text-left px-3 sm:px-4 py-2 rounded-lg font-medium transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-green-400 border-2 text-sm sm:text-base ${
            ...
            <main className="lg:w-2/3 w-full">
              {filteredCategories.map((cat) => (
                <div
                  ...
                  id={slugify(cat)}
                  ref={(el) => {
                    sectionRefs.current[slugify(cat)] = el;
                  }}
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
                    {providersByCategory[cat].map((provider, idx) => (
                      <div
                        ...
                        onClick={() =>
                          router.push(
                            `/service/${encodeURIComponent(cat)}/${idx}`
                          )
                        }
                      >
                        <div className="flex items-center mb-3 w-full">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="flex items-center gap-2 text-yellow-500 text-xs font-semibold mt-1">
                              {Array(Math.round(provider.rating))
                                .fill(0)
                                .map((_, i) => (
                                  <span key={i}>★</span>
                                ))}
                        ...
                        <div className="text-gray-700 text-xs sm:text-sm mb-2 w-full line-clamp-2">
                          {provider.desc}
                        </div>
